/* tools/webp-bridge.js ― ブラウザで作ったWebPを受け取ってファイルに書く小さな窓口
   ...
   - 拡張子は .webp と .jpg だけ（ヒーローの背景がJPEGなので後から足した）
*/
'use strict';
...
const server = http.createServer(function (req, res) {
  ...
  const okExt = target.endsWith('.webp') || target.endsWith('.jpg');
  if (!name || name.includes('..') || !target.startsWith(ROOT) || !okExt) {
  ...
  req.on('end', function () {
    ...
    /* 中身が本当にその形式かを確かめてから書く。
       WebP は RIFF....WEBP、JPEG は FF D8 で始まり FF D9 で終わる。 */
    const isWebp = buf.length >= 16 && buf.slice(0, 4).toString() === 'RIFF'
                && buf.slice(8, 12).toString() === 'WEBP';
    const isJpeg = buf.length >= 4 && buf[0] === 0xFF && buf[1] === 0xD8
                && buf[buf.length - 2] === 0xFF && buf[buf.length - 1] === 0xD9;
    if (!(target.endsWith('.webp') ? isWebp : isJpeg)) {
      res.writeHead(400); res.end('中身が拡張子と合わない'); return;
    }
...
server.listen(PORT, '127.0.0.1', function () {
  console.log('画像の受け取り口: http://127.0.0.1:' + PORT + '/save?name=<相対パス>.webp|.jpg');
});
