<#
  build-hero-wide.ps1 ―【現在は使っていない】

  2026-09-02 以降、横画面の昼の絵は依頼主からもらった
  「舟が最初から描かれている絵」をそのまま使っている
  （tools/build-hero-night.ps1 が寸法を揃えて書き出す）。
  この script が作っていた合成版は、夜の絵と舟の並びが揃わないため
  差し替えた。

  縦画面の昼（hero-mobile.jpg）はいまも合成で作っているので、
  そちらは tools/build-hero-mobile.ps1 が現役。
  ここに残してあるのは、水の地図・重なり判定の作り方の記録として。
